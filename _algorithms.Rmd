         ...
         for (ite in 1:maxIte) {
            ...
            for (ss in states) {  # for episode with s as start
               ...
               for (i in nrow(df):1) {
                  ...
                  newV <- self$getStateValue(s)
                  delta <- max(delta, abs(oldV - newV))
                  if (verbose) df$oldQ[i] <- oldQ; df$q[i] <- self$model[[s]]$actions[[a]]$q; df$g[i] <- gain; df$nA[i] <- ctr; df$nS[i] <- self$model[[s]]$n; df$oldV[i] <- oldV; df$v[i] <- self$getStateValue(s)
               }
               if (verbose) print(df)
            }
