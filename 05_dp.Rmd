...
MDPClass <- R6Class("MDPClass",
   public = list(
      ...
      getStateValues = function() {
         enframe(values(self$v), name = "state", value = "v") %>% 
            separate(state, c("state", "action")) %>% 
            select(-action)
      },
      ...
      policyIte = function(gamma, theta = 0.00001, maxIteEval = 10000, maxItePolicy = 100) {
         ...
         message("Policy iteration algorihm finished.")
         return(invisible(NULL))
      },
      ...
      valueIte = function(gamma, theta = 0.00001, maxIte = 10000, reset = TRUE) {
         ...
         message("Value iteration algorihm finished.")
         return(invisible(NULL))
      }
   )
)
...
Let us try to evaluate a fixed policy given a discount factor of 0.5:
...
Finally, let us try to solve the problem with a discount factor of 0.99:
...
Consider the car rental problem in Exercise \@ref(ex-mdp-2-car). Note the inventory dynamics (number of cars) at each parking lot is independent of the other given an action $a$. Let us consider Location 1 and assume that we are in state $x$ and chose action $a$. Then the number of cars after movement is $x - a$ and after rental requests $x - a - \min(D_1, x-a)$. Next, the number of returned cars are added: $x - a - \min(D_1, x-a) +  H_1$. Finally, note that if this number is above 20 (parking lot capacity), then we only have 20 cars, i.e. the inventory dynamics (number of cars at the end of the day) is $$X = \min(20,  x-a - \min(D_1, x-a) +  H_1))).$$. 
Similar for Location 2 we have $$Y = \min(20, y + a - \min(D_2, y+a) + H_2)).$$
...
$$\begin{align} p(x | x', a) &= \Pr(x' = x-a - \min(D_1, x-a) +  H_1)\\ &= \Pr(H_1 - \min(D_1, x-a) = x' - x + a) \\ &= \sum_{i=0}^{20} \Pr(\min(D_1, x-a) = i)\Pr(H_1 = x' - x + a + i) \\ &= \sum_{i=0}^{20} \Pr(\min(D_1, x-a) = i)\phi_{H_1}(x' - x + a + i) \\ &= \sum_{i=0}^{20}\left( (\sum_{j=0}^{x-a} j \phi_{D_1}(j) + (1-\Pr(D_1 \leq x-a))(x-a))\Pr(\min(D_1, x-a) = i)\phi_{H_1}(x' - x + a + i)\right)\end{align}$$ 
...
```{r car-transPr, include=FALSE, eval=FALSE}
calcTransPr <- function() {
   dD <- rbind(dpois(0:20, 3), dpois(0:20, 4))   # density pr(D_i = d) = dD[i, d+1]
   pD <- rbind(ppois(0:20, 3), ppois(0:20, 4))   # distribution pr(D_i <= d) = pD[i, d+1]
   dH <- rbind(dpois(0:20, 3), dpois(0:20, 2))   # density pr(H_i = d) = dH[i, d+1]
   pH <- rbind(ppois(0:20, 3), ppois(0:20, 2))   # distribution pr(D_i <= d) = pD[i, d+1]
   
   # Pr(min(D_i, k) = d)
   prMin <- function(i, k, d) {
      v <- c(dD[i, 0:(k-1) + 1], 1 - pD[i, k])
      return(v[d+1])
   ...
   # Pr(xN | x, a)
   transPrX <- function(xN, x, a) {
      if (xN == 20) {
         return(sum(prMin(1, x-a, 0:(x-a)) * (1 - pH[1, xN - x + a + 0:(x-a) ])))
      }
      if (xN < 20) {
         return(sum(prMin(1, x-a, 0:(x-a)) * dH[1, xN - x + a + 0:(x-a) + 1]))
      }
      error("Error in calculations!")
   }
   
   # Pr(yN | y, a)
   transPrY <- function(yN, y, a) {
      if (yN == 20) {
         return(sum(prMin(2, y+a, 0:(y+a)) * (1 - pH[2, yN - y - a + 0:(y+a) ])))
      }
      if (yN < 20) {
         return(sum(prMin(2, y+a, 0:(y+a)) * dH[2, yN - y - a + 0:(y+a) + 1]))
      }
      error("Error in calculations!")
   }
   
   dat <- expand_grid(x = 0:20, y = 0:20, xN = 0:20, yN = 0:20, a = -20:20) %>% 
      filter(a >= -min(5, y, 20-x), a <= min(5, x, 20-y)) %>% 
      mutate(pr = 0) 
   for (i in 1:nrow(dat)) {
      dat$pr[i] <- transPrX(dat$xN[i], dat$x[i], dat$a[i]) * transPrY(dat$yN[i], dat$y[i], dat$a[i])
   }
   return(dat)
}
dat <- calcTransPr()
...
2) Solve the problem using policy iteration with $p_h = 0.4$. What is the optimal policy and the optimal action in state 4 m$^2$? What is the expected total discounted cost of being in state 2 m$^2$.

3) Solve the problem using value iteration with a discount rate of 0.5. Check that the policy and state-values are the same.

4) Solve the problem with a discount rate of 0.99. Why are the state-values lower now?
